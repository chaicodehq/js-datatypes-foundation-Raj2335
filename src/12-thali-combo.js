...
export function createThaliDescription(thali) {
  if (thali == null || typeof thali !== "object" || !thali.name || !Array.isArray(thali.items) || !thali.price || typeof thali.isVeg !== "boolean") {
    return ""
  }
  let thaliName = thali.name.toUpperCase()
  let thaliItems = thali.items.join(', ')
  let thaliPrice = thali.price.toFixed(2)

  return `${thaliName} (${thali.isVeg ? "Veg" : "Non-Veg"}) - Items: ${thaliItems} - Rs.${thaliPrice}`;
}
